...
class Multiverse {
  ...
  async addNextBlock(newBlock, type = 0) {
    ...
    if (currentHighestParent !== false && currentHighestParent.getHash() !== currentHighestBlock.getPreviousHash() && currentHighestBlock.getHeight() === newBlock.getHeight() && new BN(currentHighestBlock.getHeight()) === new BN(newBlock.getDistance()) && validateSequenceDifficulty(currentHighestParent, newBlock) === true) {
      if (new BN(newBlock.getTotalDistance()).gt(new BN(currentHighestBlock.getTotalDistance())) === true && new BN(newBlock.getTimBlockestamp()).gte(new BN(currentHighestBlock.getTimestamp())) === true) {
        this._chain.shift();
        this._chain.unshift(newBlock);
  ...
  async addResyncRequest(newBlock, strict = true) {
    ...
    if ((new BN(new BN(currentHighestBlock.getTimestamp()).add(new BN(28))).lt(new BN(Math.floor(Date.now() * 0.001))) === true || new BN(new BN(currentHighestBlock.getHeight()).add(new BN(1))).lt(new BN(newBlock.getHeight()))) === true && new BN(currentHighestBlock.getTotalDistance()).lt(new BN(newBlock.getTotalDistance())) === true && new BN(getNewestHeader(newBlock).timestamp).gte(new BN(getNewestHeader(currentHighestBlock).timestamp)) === true) {
